...
## 📞 ১৩. বিডি আইপি টেলিফোনি ও আলাপের আইপি নাম্বার ইন্টিগ্রেশন (BTCL Alaap & 096 IP TSP)

- **বিডি আইপি টেলিফোনি গেটওয়ে (09696 / 096xx)**: BTCL Alaap, Amber IT, Brilliant Connect এবং যেকোনো স্ট্যান্ডার্ড SIP Trunk credentials (SIP Server, Username, Password, API Key) কনফিগার করার সুবিধা।
- **কলার আইডি কন্ট্রোল**: গ্রাহকের ফোনে সরাসরি দোকানের অফিশিয়াল `09696xxxxxx` বা `096xxxxxxxx` আইপি নাম্বার কলার আইডি হিসেবে প্রদর্শন।
- **১-ক্লিক আলাপে কল ওপেন (Web-to-Alaap URI)**: অ্যাডমিন ডায়ালারে ১-ক্লিকে আলাপের ডায়ালার বা ফোন ডায়ালারে সরাসরি কাস্টমারের নাম্বার তুলে কল করার সুবিধা।

---

## 🤖 ১৪. এন্টারপ্রাইজ এআই ৩.০ — সোশ্যাল অ্যাড কপি জেনারেটর ও ফ্রড ট্রাস্ট রিস্ক স্কোরিং (AI Marketing & Fraud Shield)

- **AI সোশ্যাল মার্কেটিং ও অ্যাড কপি জেনারেটর (`AiMarketingCopyService`)**: স্টোরের যেকোনো প্রোডাক্টের জন্য ১-ক্লিকে হাই-কনভার্টিং ফেসবুক অ্যাড কপি (হুক, ইমোজি, বুলেট পয়েন্ট, অফার প্রাইস, সিটিএ বাটন), ইনস্টাগ্রাম ট্রেন্ডি ক্যাপশন ও হ্যাশট্যাগ এবং এসএমএস ব্রডকাস্ট মেসেজ স্বয়ংক্রিয়ভাবে তৈরি করার সুবিধা।
- **AI অর্ডার ফ্রড ও ট্রাস্ট রিস্ক স্কোরিং (`AiFraudScoreService`)**: প্রতিটি অর্ডারের কাস্টমার ফোন নম্বর স্ট্রাকচার, বিস্তারিত ঠিকানা ও পূর্বের ডেলিভারি/বাতিল রেকর্ড বিশ্লেষণ করে স্বয়ংক্রিয় **AI Trust Score (০-১০০%)** ও করণীয় পরামর্শ প্রদান।
- **অর্ডার ডিটেইলসে লাইভ ফ্রড শিল্ড কার্ড**: অর্ডার পেজে সরাসরি সবুজ/হলুদ/লাল রঙের ট্রাস্ট ব্যাজ এবং বুকিংয়ের জন্য এআই রিকমেন্ডেশন প্রদর্শন।
- **ইন-ব্রাউজার ভার্চুয়াল সফটফোন কিপ্যাড (WebRTC Softphone Keypad)**: ডায়ালপ্যাড (১-৯, *, ০, #), সাম্প্রতিক অর্ডার অটো-পিকার ও রিয়েল-টাইম সাউন্ড ওয়েভ অ্যানিমেশন।

---

## 📱 ১৫. WhatsApp বিজনেস ক্লাউড কানেকশন ও স্বয়ংক্রিয় এআই চ্যাট অর্ডার কনফার্মেশন (WhatsApp Cloud & Webhooks)

- **WhatsApp বিজনেস নম্বর ও ক্লাউড গেটওয়ে**: Meta WhatsApp Cloud API (Phone Number ID, Cloud Token, Webhook URL & Verify Token) ইন্টিগ্রেশন।
- **স্বয়ংক্রিয় চ্যাট ও অর্ডার কনফার্মেশন**: কাস্টমার হোয়াটসঅ্যাপে মেসেজ পাঠালে (`/api/whatsapp/webhook`) এআই বট স্বয়ংক্রিয়ভাবে রিপ্লাই দেয় এবং 'হ্যাঁ' বা 'confirm' বললে স্বয়ংক্রিয়ভাবে Steadfast কুরিয়ারে বুকিং করে ট্র্যাকিং আইডি কাস্টমারকে পাঠিয়ে দেয়।
- **ভয়েস কলিং ও ডায়ালার পেজের ডিজাইন রিফ্যাক্টরিং**: ডায়ালার ট্যাবের ডুপ্লিকেট কন্টেইনার এবং সিএসএস লেআউট ফিক্স করে সম্পূর্ণ গ্লাস-ডিজাইন ও রেসপন্সিভ করা হয়েছে।

---

## 🧹 ১৬. এআই হাব সিম্প্লিফিকেশন ও ক্লিন কন্ট্রোল সেন্টার (Clean & Minimalist AI Suite)

- **অপ্রয়োজনীয় জটিল এপিআই ও টোকেন রিমুভাল**: ব্যবহারকারীর ফিডব্যাক অনুযায়ী জটিল SIP/Twilio credentials, Meta Cloud Tokens ও অতিরিক্ত সাব-ট্যাব বাদ দিয়ে এআই মডিউলকে একদম সহজ ও ছিমছাম করা হয়েছে।
- **সিঙ্গেল পেজ এআই ড্যাশবোর্ড**: একটি মাত্র ক্লিন পেজে **এআই সেলস চ্যাটবট সেটিংস**, **১-ক্লিক ফেসবুক অ্যাড কপিরাইটার** এবং **গুগল অটো-এসইও জেনারেটর** নিয়ে আসা হয়েছে।
- **ব্যবহারবান্ধব ও ফাস্ট**: কোনো জটিল টেকনিক্যাল সেটআপ ছাড়াই অ্যাডমিন যে কোনো পণ্যের ফেসবুক বিজ্ঞাপন তৈরি করতে পারেন এবং স্টোরের চ্যাটবট নিয়ন্ত্রণ করতে পারেন।

---

## 💬 ১৭. ওয়েবসাইটে ডিরেক্ট ফ্লোটিং WhatsApp মেসেঞ্জার বাটন (Floating WhatsApp Messenger)

- **স্টোরফ্রন্ট ফ্লোটিং বাটন**: ওয়েবসাইটের নিচের ডানপাশে এআই বটের পাশাপাশি একটি সুন্দর সবুজ রঙের ব্লিঙ্কিং WhatsApp বাটন যুক্ত করা হয়েছে।
- **১-ক্লিক ডিরেক্ট চ্যাট**: কাস্টমার যেকোনো পেজ থেকে ক্লিক করলেই সরাসরি স্টোরের WhatsApp নম্বরে সালাম ও মেসেজ সহ চ্যাট উইন্ডো খুলে যায়।
- **অ্যাডমিন থেকে নম্বর নিয়ন্ত্রণ**: অ্যাডমিন এআই হাব (`/admin/ai-automation`) থেকে সরাসরি WhatsApp নম্বর পরিবর্তন করতে পারেন।
